...
{
    public partial class CallDueRpt : System.Web.UI.Page
    {
        ...
        private void GenerateReport()
        {
            ...
            LocalReportManager reportManager = new LocalReportManager(rptViewer, "CallDueRpt", ConfigurationManager.AppSettings["ReportNamespace"].ToString(), ConfigurationManager.AppSettings["ReportPath"].ToString());
            string rptName = "CallDueRpt.rdlc";
            DateTime fromDate;
            DateTime toDate;
            fromDate = Convert.ToDateTime(txtFromDt.Text, _culture);
            toDate = Convert.ToDateTime(txtToDt.Text, _culture);
            BuildEntity(callDetail);
            IEnumerable<ICallDetail> lst = cls.GetCallDueData(fromDate, toDate, callDetail, _userId);
