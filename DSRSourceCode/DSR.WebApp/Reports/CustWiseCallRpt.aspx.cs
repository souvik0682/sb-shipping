...
{
    public partial class CustWiseCallRpt : System.Web.UI.Page
    {
        ...
        private void GenerateReport()
        {
            ...
            LocalReportManager reportManager = new LocalReportManager(rptViewer, "CustomerWiseCallRpt", ConfigurationManager.AppSettings["ReportNamespace"].ToString(), ConfigurationManager.AppSettings["ReportPath"].ToString());
            string rptName = "CustomerWiseCallRpt.Rdlc";
            DateTime fromDate;
            DateTime toDate;
            fromDate = Convert.ToDateTime(txtFromDt.Text, _culture);
            toDate = Convert.ToDateTime(txtToDt.Text, _culture);
            BuildEntity(callDetail);
            IEnumerable<ICallDetail> lst = cls.GetCustomerWiseCallData(fromDate, toDate, callDetail, _userId);
